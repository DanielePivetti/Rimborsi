    ...
    if form.validate_on_submit():
        ...
        try:
            ...
            if tipo_spesa == TipoSpesa.CARBURANTE.value:
                ...
                if not indirizzo:
                    flash('Inserisci l\'indirizzo del parcheggio', 'danger')
                    return redirect(url_for('spesa.aggiungi_spesa', richiesta_id=richiesta_id))
                
                if not durata_ore:
                    flash('Inserisci la durata del parcheggio (ore)', 'danger')
                    return redirect(url_for('spesa.aggiungi_spesa', richiesta_id=richiesta_id))
                ...
                if not descrizione_dettagliata:
                    flash('Inserisci una descrizione dettagliata per la spesa', 'danger')
                    return redirect(url_for('spesa.aggiungi_spesa', richiesta_id=richiesta_id))
...
def documenti_spesa(spesa_id):
    ...
    # Aggiunge campi dinamici per nuovi documenti
    for _ in range(3):
        form.documenti.append_entry()
    
    if form.validate_on_submit() and modifica_consentita:
        try:
            # Gestisce l'upload di ciascun documento
            for doc_form in form.documenti:
                # Verifica se un file è stato caricato in questo campo
                if doc_form.file.data and doc_form.file.data.filename:
                    # Genera un nome univoco per il file
                    nome_originale = secure_filename(doc_form.file.data.filename)
                    ...
                    doc_form.file.data.save(percorso_file)
                    ...
                    documento = DocumentoSpesa(
                        ...
                        nome_file=nome_file,
                        note=doc_form.note.data
                    )
                    db.session.add(documento)
            
            db.session.commit()
            flash('Documenti caricati con successo', 'success')
            return redirect(url_for('spesa.documenti_spesa', spesa_id=spesa_id))
        
        except Exception as e:
            db.session.rollback()
            flash(f'Errore durante il caricamento dei documenti: {str(e)}', 'danger')
